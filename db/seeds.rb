def extract_url_from_file(file_path)
  File.read(file_path).split('"').filter { |item| item.start_with? "http" }
end
...
urls = extract_url_from_file(Rails.root.to_s + "/db/urls.txt").uniq
...
urls.each do |url|
  ...
  product_hash = JSON.parse(product_raw)["product"] rescue next
  next if product_hash.nil?
